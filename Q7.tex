...
\end{figure}
    ...
    \vs \\
    The Laurent Series of $f$ about -1 will contain $a_n \neq 0$ for infinitely many $n<0$, therefore it has an essential singularity at -1, by HB B4 2.10(c), p57. 
    \vs \\
    Thus, by the Casorati–Weierstrass Theorem, on the punctured disc $ D =  \{z: 0 < |z+1| < 1 \} $, $ |f(z)| $ will get arbitrarily close to any arbitrarily large number. In other words it is unbounded on $ D \in C - \{  -1 \} $.
    ...
    % \textcolor{purple}{Ok, I put your solution as the official one Donach. Thanks! -- Thomas}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \]
        by the Cauchy-Riemann Converse Theorem. However, the Cauchy-Riemann equations fail at points other than $\left(0, \frac{1}{2}\right)$, so $f$ is not differentiable on $ \C - \big\{\left(0, \frac{1}{2}\right)\big\}$ by the Cauchy-Riemann Theorem.
        \end{enumerate}
        % {
        % \color{teal}
